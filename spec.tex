...
{\bf NEW: } Assume that {\tt bad\_hash} only has to work with string input.  (Of course, it should fail gracefully if not.)
...
\begin{description}
    \item {\bf E) Implementing universal hashing: }
        ...
        generate a list of random integers $r_0, r_1, \ldots, r_{{\tt MAXSTRLEN}-1}$
...
\begin{description}
    \item[\bf E) Generating collisions by trial and error:]
...
{\bf NEW: } Output format: write your output to {\tt sdtout.}  It should contain:
\begin{itemize}
\item {\tt Num\_randoms} (The number of {\tt r[i]} values to appear), followed by $'\backslash n'$
\item the random elements {\tt r[0], r[1],} $\ldots,$ {\tt r[Num\_randoms-1]} in order, one per line,
...
\begin{description}
\item[\bf F) Generating collisions by clever maths:]
...
\begin{enumerate}
\item Assume that {\tt size} is less than the maximum value of char.  (so at most 255)
...
Partial credit also for an algorithm that works for many, but not all, possible values of {\tt r[0], r[1], \ldots}.

{\bf NEW: } Output format: same as (E).
...
The seven coding parts (A0 through F) of the assignment will be marked for correctness and clarity.
